    ...
    useEffect(() => {
        timer.current = setInterval(async () => {
            ...
            const { data } = await indexer
                ...
                .single()
            // .eq('context', 'proof-of-networking')
            // .order('last_message_timestamp', { ascending: false })
            // const { data } = await indexer.from('orbis_v_conversations').select().eq('context', 'proof-of-networking')
            ...
            if (data) {
                ...
                if (conversationId) {
                    ...
                    // the other person receives and verifies the message, then the PoN is created and both add themselves as contacts
